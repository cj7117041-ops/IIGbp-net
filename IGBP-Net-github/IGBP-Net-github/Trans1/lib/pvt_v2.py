...
class PyramidVisionTransformerV2(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x_4x, H, W = self.patch_embed1(x)
        for blk in self.block1:
            x_4x = blk(x_4x, H, W)
        x_4x = self.norm1(x_4x)
        x_4x = x_4x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        x_8x, H, W = self.patch_embed2(x_4x)
        for blk in self.block2:
            x_8x = blk(x_8x, H, W)
        x_8x = self.norm2(x_8x)
        x_8x = x_8x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        x_16x, H, W = self.patch_embed3(x_8x)
        for blk in self.block3:
            x_16x = blk(x_16x, H, W)
        x_16x = self.norm3(x_16x)
        x_16x = x_16x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        x_32x, H, W = self.patch_embed4(x_16x)
        for blk in self.block4:
            x_32x = blk(x_32x, H, W)
        x_32x = self.norm4(x_32x)
        # x_32x = x_32x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        return x_32x.mean(dim=1)
